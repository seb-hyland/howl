use crate::vm::{runtime::Runtime, value::Value};
...
fn define_int_add(rt: &mut Runtime, id: u64) {
...
fn define_int_sumall(rt: &mut Runtime, id: u64) {
    fn handler(rt: &mut Runtime, arg_count: u64) -> Option<Value> {
        let mut sum = 0;
...
fn define_int_mul(rt: &mut Runtime, id: u64) {
    fn handler(rt: &mut Runtime, arg_count: u64) -> Option<Value> {
        if arg_count != 1 {
            panic!("Bad args!")
        }
...
fn define_int_display(rt: &mut Runtime, id: u64) {
    fn handler(rt: &mut Runtime, arg_count: u64) -> Option<Value> {
        if arg_count != 0 {
            panic!("Bad args!")
        }
